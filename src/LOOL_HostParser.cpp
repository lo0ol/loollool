...
LOOL_HostParser::LOOL_HostParser()
{
...
uint8_t LOOL_HostParser::getPackageReady()
{
    return (1 == packageReady);
}

uint8_t LOOL_HostParser::pushStr(uint8_t * str, uint32_t length)
{
...
uint8_t LOOL_HostParser::pushByte(uint8_t ch)
{
    if (((in + 1) & MASK) != out)
    {
        buffer[in] = ch;
...
uint8_t LOOL_HostParser::getByte(uint8_t * ch)
{
    if (in != out)
    {
        *ch = buffer[out];
...
uint8_t LOOL_HostParser::run(void)
{
    uint8_t ch = 0;
    while (getByte(&ch))
    {
...
uint8_t LOOL_HostParser::getData(uint8_t *buf, uint32_t size)
{
