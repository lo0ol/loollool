...
void LOOL_IR::space(uint16_t us)
{
...
void LOOL_IR::enableIROut(uint8_t khz)
{
  TIMER_DISABLE_INTR; //Timer2 disable Interrupt
  TIMER_CONFIG_KHZ(khz);
}

void LOOL_IR::sendRaw(unsigned int buf[], int len, uint8_t hz)
{
  enableIROut(hz);
  for (int i = 0; i < len; i++)
  {
...
void LOOL_IR::sendString(String s)
{
  unsigned long l;
  uint8_t data;
  s.concat('\n');
...
void LOOL_IR::sendString(float v)
{
  dtostrf(v,5, 2, floatString);
  sendString(floatString);
}
void LOOL_IR::sendNEC(unsigned long data, int nbits)
{
  enableIROut(38);
  mark(NEC_HDR_MARK);
  space(NEC_HDR_SPACE);
...
boolean LOOL_IR::keyPressed(unsigned char r)
{
  irIndex = 0;
