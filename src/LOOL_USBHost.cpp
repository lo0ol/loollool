 ...
 * @class LOOL_Wifi
 * @brief    Driver for LOOL USBHost device.
 * @file    LOOL_Wifi.h
 * @author  sean
 * @version V1.0.0
 * @date    2015/09/09
 * @brief   Header for for USBHost Module
 *
 * @par Copyright
 ...
 *
 * @par Description
 * This file is a drive for LOOL USBHost device, The USBHost inherited the 
...
LOOL_USBHost::LOOL_USBHost() : LOOL_Port(0)
{

}

/*LOOL_USBHost::LOOL_USBHost(uint8_t s1, uint8_t s2)
{
  HSerial = new SoftwareSerial(s1,s2);
}*/

LOOL_USBHost::LOOL_USBHost(uint8_t port) : LOOL_Port(port)
{

}

uint8_t LOOL_USBHost::CH375_RD()
{
...
void LOOL_USBHost::CH375_WR(uint8_t c)
{
  HSerial->write(c);
  delay(2);
#ifdef CH375_DBG
...
int16_t LOOL_USBHost::set_usb_mode(int16_t mode)
{
...
uint8_t LOOL_USBHost::issue_token( uint8_t endp_and_pid )
{
...
void LOOL_USBHost::wr_usb_data( uint8_t len, uint8_t *buf )
{
#ifdef CH375_DBG
...
uint8_t LOOL_USBHost::rd_usb_data( uint8_t *buf )
{
  uint8_t i, len;
...
int16_t LOOL_USBHost::get_version(){
  CH375_WR(CMD_GET_IC_VER);
  return CH375_RD();
}

void LOOL_USBHost::set_freq(void)
{
  CH375_WR(0x0b);
  CH375_WR(0x17);
  CH375_WR(0xd8);
...
uint8_t LOOL_USBHost::set_addr( uint8_t addr )
{
  uint8_t irq;
...
uint8_t LOOL_USBHost::set_config(uint8_t cfg){
...
uint8_t LOOL_USBHost::clr_stall6(void)
{
...
uint8_t LOOL_USBHost::get_desr(uint8_t type)
{
...
uint8_t LOOL_USBHost::host_recv()
{
  uint8_t irq;
  toggle_recv();
...
void LOOL_USBHost::init(int8_t type)
{
  ch375_online = false;
  device_online = false;
  device_ready = false;
...
int16_t LOOL_USBHost::initHIDDevice()
{
...
int16_t LOOL_USBHost::probeDevice()
{
  int16_t c;
  if(!ch375_online){
